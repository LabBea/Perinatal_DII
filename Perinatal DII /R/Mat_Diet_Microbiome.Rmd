...
bray_dist$Average_dist <- bray_dist$Average_dist/48 # divide by 48 to not include self measure

av_bray_dist <- bray_dist[,49:50]
...
MRcoefs_finalDII$Plot_names <- c("italic('g_Prevotella')", 
                                 ...
                                 "italic('g_Clostridium sensu stricto')")
...
MRcoefs <- MRcoefs %>% subset(adjPvalues < 0.1)
...
rownames(KO_parameter_fitzig) <- c(
          ...
          "ABC - Diacetylchitobiose transport (K17330)", 
          "Formate transporter (K06212)", 
          "NAD(P)H dehydrogenase (quinone) (K03809)")
...
#Step 1: Read in the Data
################################################################
GSEA_data <- read_excel(here("Output", "GSEA_Output", "GSEA_Results.xlsx"))

#Relabel the ratios for plotting
GSEA_data$GeneRatio <- c(6/101, 12/101, 4/101, 4/101)
